    ...
    (fun ri ->
       ...
       if r then
         Lwt_log.ign_info_f ~section
           "METHOD: %a matches %s"
           (fun () ri ->
            Framepp.string_of_method (Ocsigen_request_info.meth ri))
           ri s
       else
         Lwt_log.ign_info_f ~section
    ...
    (fun ri ->
       ...
       if r then
         Lwt_log.ign_info_f ~section
           ...
           (fun () ri ->
            Framepp.string_of_proto (Ocsigen_request_info.protocol ri))
           ri s
       else
         Lwt_log.ign_info_f ~section
    ...
    let apply request code =
      ...
      let request =
        try
          ...
          | []
          | [_] -> Lwt_log.ign_info_f ~section "Malformed X-Forwarded-For \
                                                field: %s" header;
            request
          | original_ip::proxies ->
            ...
            let equal_ip = proxy_ip =
                           Lazy.force (Ocsigen_request_info.remote_ip_parsed
                                         request.request_info) in
            let need_equal_ip =
              match param with
              | [] -> false
              | ["check-equal-ip",b] ->
            ...
            then
              { request with request_info =
                               (Ocsigen_request_info.update request.request_info
                                  ...
                                  ~remote_ip_parsed:(lazy (Ipaddr.of_string_exn
                                                             original_ip))
                                  ~forward_ip:proxies ()) }
            else (* the announced ip of the proxy is not its real ip *)
              (Lwt_log.ign_warning_f ~section
                  "X-Forwarded-For: host ip ( %a ) does not match \
                   the header ( %s )"
                  (fun () -> Ocsigen_request_info.remote_ip)
